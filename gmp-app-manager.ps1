...
function Stop-ProcessOnPort {
    ...
    if ($connections) {
        foreach ($conn in $connections) {
            try {
                $owningProcess = (Get-NetTCPConnection -LocalPort $Port -ErrorAction SilentlyContinue | Select-Object -First 1).OwningProcess
                if ($owningProcess) {
                    ...
                    if ($process -and $process.Id -gt 4 -and $systemProcesses -notcontains $process.ProcessName) {
                        ...
                        try {
                            ...
                            Write-Host "Impossible d'arrêter le processus $($process.ProcessName) (PID: $($process.Id)): $($_.Exception.Message)" -ForegroundColor Red
                        }
                    } else {
                        Write-Host "Processus système détecté ($($process.ProcessName), PID: $($process.Id)), ignoré." -ForegroundColor Magenta
                    }
                }
            } catch {
                Write-Host "Erreur lors de l'identification du processus: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
    }
    else {
